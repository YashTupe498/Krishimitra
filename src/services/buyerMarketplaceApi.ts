...
export const buyerMarketplaceApi = {
  getRequirements: async () => (await request<any[]>('/api/v1/buyer/demands/')).map(requirement),
  createRequirement: async (input: BuyerRequirement) => {
    ...
    if (input.status === 'ACTIVE') saved = await request(`/api/v1/buyer/demands/${saved.id}/publish`, { method: 'POST' });
    notifyBuyerDataChanged();
    return requirement(saved);
  },
  updateRequirement: async (id: string, changes: Partial<BuyerRequirement>) => {
    const updated = requirement(await request<any>(`/api/v1/buyer/demands/${id}`, { method: 'PATCH', body: JSON.stringify({ status: changes.status }) }));
    notifyBuyerDataChanged();
    return updated;
  },
  ...
  getMatches: async (requirementId: string) => (await request<any[]>(`/api/v1/buyer/demands/${requirementId}/matches`)).map((item) => match(requirementId, item)),
  getOffersByBuyer: async (): Promise<Offer[]> => (await request<any[]>('/api/v1/buyer/offers/')).map(offer),
  getOffersByFarmer: async (): Promise<Offer[]> => (await request<any[]>('/api/v1/buyer/offers/received/')).map(offer),
  ...
  respondToOffer: async (id: string, response: 'ACCEPTED' | 'REJECTED'): Promise<Offer> => offer(await request<any>(`/api/v1/buyer/offers/received/${id}?response=${response}`, { method: 'PATCH' })),
};
